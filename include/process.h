...
void preComputeGroupData( game *aGame );

void removeDeadPlayer( game *aGame );

void
doOrders( game *aGame, player *aPlayer, orderinfo *orderInfo, int phase );

int runTurn( game *aGame, char *ordersFileName );

#if FS_NEW_FORECAST
void checkOrders( game *aGame, char *raceName );
#else
void checkOrders( game *aGame, char *raceName, FILE *forecast, int kind );
#endif
...
generateErrorMessage( int resNumber,
                      game *aGame,
                      char *raceName,
                      int theTurnNumber, FILE *forecast, char *galaxyline );
