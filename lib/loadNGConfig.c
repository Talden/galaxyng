...
readDefaults( game *aGame, FILE *f )
{
    ...
    if ( aGame->serverOptions.SERVERemail == NULL ) {
        char *logfile =
            createString( "%s/Games/log/%s", galaxynghome, aGame->name );
        FILE *logfp = fopen( logfile, "+w" );
        fprintf( stderr,
                 "SERVERemail is a required entry in your .galaxyngrc\n" );
        fprintf( logfp,
                 "SERVERemail is a required entry in your .galaxyngrc\n" );
        fclose( logfp );
        exit( EXIT_FAILURE );
...
loadNGConfig( game *aGame )
{
    ...
    sprintf( lineBuffer, "%s/data/%s/.galaxyngrc", galaxynghome,
             aGame->name );
    if ( ( f = fopen( lineBuffer, "r" ) ) ) {
        readDefaults( aGame, f );
        fclose( f );
