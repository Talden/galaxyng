...
createMailToAllHeader( game *aGame )
{
    ...
    if ( aGame->serverOptions.SERVERemail ) {
        if ( aGame->serverOptions.SERVERname ) {
            printf( "From: %s <%s>\n", aGame->serverOptions.SERVERname,
...
#ifndef WIN32
    if ( e->compress &&
         aGame->serverOptions.compress && aGame->serverOptions.encode ) {
...
addMimeUUE( FILE *mailFile )
{
    ...
    fprintf( mailFile,
             "Content-Disposition: attachment; filename=\"turn.zip\"\n" );
...
void
gmNotify( char *subject, char *filename, game *aGame )
{
    envelope *env;
