    ...
    for ( isRead = fgets( lineBuffer, LINE_BUFFER_SIZE, specfile );
        ...
        if ( key[0] != '\0' ) {
            if ( noCaseStrcmp( "player", key ) == 0 ) {
                ...
                for ( value = getstr( 0 ), aNewPlayer->numberOfHomePlanets =
                      0; value[0] != '\0';
                      value =
                      getstr( 0 ), aNewPlayer->numberOfHomePlanets++ );
                if ( aNewPlayer->numberOfHomePlanets ) {
                    int i;

                    aNewPlayer->coreSizes =
                        malloc( aNewPlayer->numberOfHomePlanets *
                                sizeof( double ) );
...
creategame( gamespecification *aGameSpec )
{
    if ( createGameDirectories( aGameSpec->name ) ) {
        ...
        srand( ( unsigned ) time( NULL ) );
        resetErnie( 197162622u + ( rand(  ) & 0xFF ) );
...
Add_Extra_Home_Planets( game *aGame,
                        double x, double y,
                        double size,
                        double max_radius, int *planet_name, player *aPlayer )
{
    planet *p;
                                ...
                                double y, int *planet_name, int min_dist )
{
    int pno;
    planet *p;
    float f_min_dist = ( float ) min_dist;
