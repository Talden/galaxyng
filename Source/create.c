                ...
                while ( fgets( lineBuffer, LINE_BUFFER_SIZE, specfile ) ) {
                    ...
                    if ( noCaseStrcmp( key, "end_player" ) == 0 ) {
                        ...
                        if ( aNewPlayer->numberOfHomePlanets ==
                            ...
                            memset( &aNewPlayer->coreXOffset[lastAllocSize],
                                    0, lastAllocSize * sizeof( double ) );
                            memset( &aNewPlayer->coreYOffset[lastAllocSize],
                                    0, lastAllocSize * sizeof( double ) );
                        }
...
#if 0
                        printf( "%2d) %.2f %.2f %.2f\n", i,
                                aNewPlayer->coreSizes[i],
                                aNewPlayer->coreXOffset[i],
                                aNewPlayer->coreYOffset[i] );
#endif
                    }
                }
...
creategame( gamespecification *aGameSpec )
{
    if ( createGameDirectories( aGameSpec->name ) ) {
        ...
        if ( aGameSpec->teamGame ) {
            created_ok =
                createCheckeredLayout( aGameSpec, aGame, planet_name );
            if ( created_ok )
                setTeamAllies( aGame->players );
        } else {
            created_ok =
                createStandardLayout( aGameSpec, aGame, planet_name );
        }
        ...
        if ( created_ok ) {
            if ( aGame->gameOptions.galactic_peace > 0 ) {
                ...
                for ( aPlayer = aGame->players; aPlayer;
                    ...
                    for ( aPlayer2 = aGame->players; aPlayer2;
                          aPlayer2 = aPlayer2->next ) {
                        alliance *a;
            ...
            if ( aNewPlayer->numberOfHomePlanets > 1 ) {
                ...
                for ( cur_extra_home = 1;
                    ...
                    Add_Extra_Home_Planets( aGame, x, y,
                                            aNewPlayer->
                                            coreSizes[cur_extra_home],
                                            aGameSpec->radiusSecondaryPlanets,
                                            &planet_name, aPlayer,
                                            aNewPlayer->
                                            coreXOffset[cur_extra_home],
                                            aNewPlayer->
                                            coreYOffset[cur_extra_home] );
                }
            }
        ...
        for ( column = 0; column < aGameSpec->numberOfColumns; column++ ) {
            ...
            if ( aNewPlayer &&
                ...
                if ( aNewPlayer ) {
                    ...
                    if ( aGameSpec->numberOfEmptyPlanets ) {
                        Add_Empty_Planets( aGame,
                                           aGameSpec->numberOfEmptyPlanets,
                                           aGameSpec->radiusEmptyPlanets, x,
                                           y, &planet_name, 600.0, 700.0 );
                    }
                }
            }
            if ( ( column > 0 ) && ( row > 0 ) ) {
                ...
                Add_Extra_Home_Planets( aGame, x, y, 4000, 3.3, &planet_name,
                                        NULL, 0.0, 0.0 );
            }
            team++;
            if ( team > 1 ) {
...
Add_Circle_Stuff_Planets( game *aGame,
                          int stuff_planets, int *planet_name, double sphere )
{
    int cur_planet_nr;
    planet *p;
    double x, y, r;
                      ...
                      player *aPlayer, double size, double px, double py )
{
    ...
    if ( px > 0.0 && py > 0.0 ) {
        x = px;
        y = py;
        sprintf( unode->key, "%08d%08d", ( int ) ( x * 100 ),
                 ( int ) ( y * 100 ) );
        if ( avl_search( &unique, ( avl * ) unode, avliter ) == 0 ) {
            avl_insert( &unique, ( avl * ) unode );
        }
                   ...
                   int *planet_name, double low_range, double up_range )
{
    ...
    for ( cur_total_size = 0.0, cur_planet_nr = 0; cur_planet_nr < nplanets;
          cur_planet_nr++ ) {
