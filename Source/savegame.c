...
savegame( game *aGame )
{
    ...
    /* this next *shoud be* writeInt() */
    writeFloat( ( float ) aGame->gameOptions.gameOptions );
    ...
    for ( P = aGame->players; P; P = P->next ) {
        ...
        for ( aShipType = P->shiptypes; aShipType;
              aShipType = aShipType->next ) {
    ...
    for ( P = aGame->players; P; P = P->next ) {
        ...
        writeString( "@Allies" );
        for ( anAlliance = P->allies; anAlliance;
              anAlliance = anAlliance->next ) {
