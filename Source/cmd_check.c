...
int
mailForecast( char *forecastName,
              char *tag, envelope *anEnvelope, game *aGame, int kind )
{
    int result = FALSE;
...
int
mail_AdvanceReport( game *aGame,
                    player *aPlayer,
                    envelope *anEnvelope,
                    char *raceName,
                    int kind, enum EReportFormat report_format )
{
    int result = FALSE;
    /* TODO */
    return result;
...
int
mail_TXT_Error( game *aGame,
                envelope *anEnvelope,
                char *raceName, int kind, int resNumber, int theTurnNumber )
{
    int result = FALSE;
    FILE *forecast;
        ...
        if ( resNumber == RES_OK ) {
            ...
            if ( ( theTurnNumber == LG_CURRENT_TURN ) ||
                ...
                if ( aPlayer->flags & F_XMLREPORT ) {
                    result =
                        mail_Forecast( aGame, aPlayer, anEnvelope, raceName,
                                       kind, REP_XML );
                }
                ...
                if ( aPlayer->flags & F_XMLREPORT ) {
                    result =
                        mail_AdvanceReport( aGame, aPlayer, anEnvelope,
                                            raceName, kind, REP_XML );
                }
                if ( aPlayer->flags & F_TXTREPORT ) {
                    result =
                        mail_AdvanceReport( aGame, aPlayer, anEnvelope,
                                            raceName, kind, REP_TXT );
                }
            } else {
...
CMD_check( int argc, char **argv, int kind )
{
    ...
    if ( argc >= 2 ) {
        ...
        resNumber = areValidOrders( stdin, &aGame, &raceName,
                                    ...
                                    &galaxyline );
        /* TODO FS Hack. */
        if ( resNumber == RES_NO_REPORT_TURN_NBR ) {
            resNumber = RES_NO_ORDERS_TURN_NBR;
        }
        /* Should be fixed in areValidOrders. */
        ...
        if ( resNumber == RES_OK ) {
            ...
            generateErrorMessage( resNumber, aGame, raceName,
                                  theTurnNumber, forecast, galaxyline );
            fclose( forecast );
        ...
        if ( ( theTurnNumber > ( aGame->turn + 1 ) )
            ...
            if ( final_orders ) {
                setHeader( anEnvelope, MAILHEADER_SUBJECT,
                           ...
                           aGame->name, theTurnNumber, raceName );
                plog( LBRIEF,
                      "%s turn %d advance finalorders received for %s.\n",
                      aGame->name, theTurnNumber, raceName );
            } else {
                setHeader( anEnvelope, MAILHEADER_SUBJECT,
                           "[GNG] %s turn %d advance orders received for %s.",
