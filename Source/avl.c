 ...
 */
int
avl_range( avl_tree * t, avl * a, avl * b, int ( *iter ) ( avl * a ) )
{
    int x, c = 0;
    if ( !t->root )
        return 0;
    ...
    if ( x >= 0 ) {
        ...
        if ( ( left_subtree.root = t->root->left ) != NULL ) {
            left_subtree.compar = t->compar;
            if ( !( c = avl_range( &left_subtree, a, b, iter ) ) )
                if ( x > 0 )
                    return 0;
        }
    }
...
/*      if(right_subtree.root=t->root->right){  */
            right_subtree.compar = t->compar;
            if ( !( c = avl_range( &right_subtree, a, b, iter ) ) )
                if ( x < 0 )
                    return 0;
        }
    }
 ...
 */
int
avl_search( avl_tree * t, avl * a, int ( *iter ) ( avl * a ) )
{
