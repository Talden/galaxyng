...
CMD_relay( int argc, char **argv )
{
    ...
    if ( noCaseStrcmp( destination, aGame->name ) == 0 ) {
        ...
        if ( noCaseStrcmp( destination, "GM" ) == 0 ) {
            ...
            if ( ( itPlayer =
                   findElement( player, aGame->players,
                                destination ) ) == NULL ) {
    ...
    for ( listPlayer = toPlayers; listPlayer; listPlayer = listPlayer->next ) {
        ...
        result =
            relayMessage( aGame, anEnvelope, raceName, fromPlayer,
                          listPlayer );

        if ( result == 0 ) {
            setHeader( anEnvelope, MAILHEADER_SUBJECT,
                       "[GNG] %s message sent to %s", aGame->name,
                       listPlayer->name );
            fprintf( confirm, "Message has been sent to %s.\n",
                     listPlayer->name );
        } else {
            setHeader( anEnvelope, MAILHEADER_SUBJECT,
                       "[GNG] %s message not sent to %s", aGame->name,
                       listPlayer->name );
            fprintf( confirm,
                     "Due to a server error the message was not "
