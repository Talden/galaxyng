...
checkIntegrity( game *aGame )
{
    ...
    for ( aBattle = aGame->battles; aBattle; aBattle = aBattle->next ) {
        plog( LFULL, "o battle\n" );
...
checkFleets( player *aPlayer )
{
    ...
    for ( aFleet = aPlayer->fleetnames; aFleet; aFleet = aFleet->next ) {
...
void
bat_addPlanet( game *aGame, player *owner, int *planet_name, double x,
               double y )
{
    planet *p;
    p = addplanet( aGame );
...
player *
bat_createPlayer( game *aGame, char *name )
{
    player *aPlayer;
...
void
bat_addShiptypes( player *aPlayer )
{
    shiptype *t;
    t = bat_createShiptype( "drone", 1.0, 0, 0.0, 0.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "armed_drone", 1.0, 1, 1.0, 0.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Big_Gun", 33.0, 1, 33.0, 33.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Big_Shield", 50.0, 1, 1.0, 350.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Multi_Gun", 33.0, 5, 10.0, 36.0, 0.0 );
    addList( &aPlayer->shiptypes, t );

    t = bat_createShiptype( "Multi_Gun2", 33.0, 4, 13.2, 33.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Anti_Flak", 34.5, 98, 1.0, 15.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Anti_Flak2", 33.0, 93, 1.0, 15.0, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Mini_Anti_Flak", 16.6, 37, 1.0, 11.9, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Giant_Anti_Flak", 69.5, 219, 1.0, 19.5, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Tiny_Anti_Flak", 8.6, 12, 1.0, 9.6, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "SmFlak", 33.25, 95, 1.0, 17.62, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "Main_Gun", 34.0, 7, 10.14, 23.82, 0.0 );
    addList( &aPlayer->shiptypes, t );
    t = bat_createShiptype( "AntiFlak3", 33.2, 118, 1.0, 6.1, 0.0 );
    addList( &aPlayer->shiptypes, t );
}
...
shiptype *
bat_createShiptype( char *name, double drive, int attacks, double weapons,
                    double shields, double cargo )
{
    shiptype *t;
...
void
bat_addGroups( game *aGame, char *playerName, char *planetName,
               char *typeName, int numberOfShips )
{
    group *aGroup;
    player *aPlayer;
    planet *aPlanet;
...
void
bat_allies( game *aGame, char *playerName1, char *playerName2 )
{
    player *aPlayer1;
    player *aPlayer2;
    alliance *a;
