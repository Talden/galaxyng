            ...
            if ( g->ships ) {
                for ( g2 = P->groups; g2; ) {
                    ...
                    if ( g != g2 &&
                         ...
                         g->type == g2->type &&
                         epsilon( g->drive, g2->drive, 0.0001 ) &&
                         epsilon( g->weapons, g2->weapons, 0.0001 ) &&
                         epsilon( g->shields, g2->shields, 0.0001 ) &&
                         epsilon( g->cargo, g2->cargo, 0.0001 ) &&
                         g->loadtype == g2->loadtype &&
                         epsilon( g->load, g2->load, 0.0001 ) &&
                         g->from == g2->from &&
                         g->where == g2->where &&
                         g->flags == g2->flags
                         && epsilon( g->dist, g2->dist, 0.0001 ) ) {
                        g->ships += g2->ships;
                        remList( &P->groups, g2 );
                    }
                    g2 = g3;
        ...
        for ( attackGroup = aPlayer->groups;
            ...
            if ( mustBomb( aPlayer, attackGroup ) ) {
                ...
                if ( targetPlanet->owner ) {
                    ...
                    plog( LPART,
                          "Ownership goes to nobody, planet neutral\n" );
        ...
        if ( ( P = p->owner ) != 0 ) {
            if ( p->routes[CG_COL] )
                for ( g = P->groups; g; g = g->next )
                    if ( g->where == p &&
                         epsilon( g->type->cargo, 0.0, 0.0001 ) &&
                         epsilon( g->dist, 0.0, 0.0001 ) &&
                         epsilon( g->type->drive, 0.0, 0.0001 ) &&
                         !g->thefleet ) {
                        if ( g->loadtype == CG_EMPTY
                             && epsilon( p->col, 0.0, 0.0001 ) ) {
                            qty = p->col;
...
interceptphase( game *aGame )
{
    ...
    massPerPlanet =
        ( double * ) alloc( ( noPlanets + 1 ) * sizeof( double ) );
    ...
    for ( curPlayer = aGame->players; curPlayer; curPlayer = curPlayer->next ) {
        ...
        for ( inGroup = curPlayer->groups; inGroup; inGroup = inGroup->next ) {
            if ( inGroup->flags & GF_INTERCEPT ) {
                ...
                if ( inGroup->thefleet ) {
                    pdebug( DFULL,
                            "Fleet %s Intercept on Planet %s max Dist %f\n",
                            inGroup->thefleet->name, inPlanet->name,
                            maxDist );
                } else {
                    pdebug( DFULL,
                            "Group %d Intercept on Planet %s max Dist %f\n",
                ...
                memset( massPerPlanet, 0,
                        ( noPlanets + 1 ) * sizeof( double ) );
                ...
                for ( otherPlayer = aGame->players;
                    ...
                    if ( otherPlayer != curPlayer ) {
                        ...
                        for ( aGroup = otherPlayer->groups;
                            ...
                            if ( ( epsilon( aGroup->dist, 0.0, 0.0001 ) ) &&
                                 ( aGroup->location eq inPlanet ) &&
                                 ( dist
                                   ( aGame, aGroup->where,
                                     inGroup->location ) < maxDist ) ) {
                                massPerPlanet[ptonum
                                              ( aGame->planets,
                                                aGroup->where )] +=
                ...
                targetPlanet = NULL;
                for ( curPlanet = aGame->planets, planetIndex = 1, maxMass =
                      0; curPlanet;
                      planetIndex++, curPlanet = curPlanet->next ) {
        ...
        for ( cur_player = aGame->players;
            ...
            if ( cur_player->flags & F_AUTOUNLOAD ) {
                for ( g = cur_player->groups; g; g = g->next ) {
                    if ( g->where == p && epsilon( g->dist, 0.0, 0.0001 )
                         && g->ships ) {
                        if ( ( p->owner && p->owner == cur_player )
                             || !p->owner ) {
        ...
        if ( p->owner ) {
            for ( i = 0; i != CG_EMPTY; i++ ) {
                ...
                for ( g = p->owner->groups; g; g = g->next ) {
                    if ( g->where == p2 &&
                         epsilon( g->dist, 0.0, 0.0001 ) && g->loadtype == i
                         && g->ships ) {
...
cktech( group *g )
{
    ...
    if ( !t->attacks )
        g->weapons = 0;
    if ( !epsilon( t->shields, 0.0, 0.0001 ) )
        g->shields = 0;
    if ( !epsilon( t->cargo, 0.0, 0.0001 ) )
        g->cargo = 0;
}
...
produceShip( planet *aPlanet, double industry )
{
    ...
    for ( n =
          ( int ) ( industry / ( typeMass * INDPERSHIP ) ) + 1,
          numberOfShips = 0; n >= 0; n-- ) {
        indForProduction = n * typeMass * INDPERSHIP;
        indForMaterials =
            ( n * typeMass - aPlanet->mat ) / aPlanet->resources;
