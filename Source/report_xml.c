...
rST_XML( game *aGame, player *P )
{
    ...
    for ( P2 = aGame->players; P2; P2 = P2->next ) {
        fprintf( fpR,
                 ...
                 safename( P2->name ), P2->drive, P2->weapons, P2->shields,
                 P2->cargo, P2->totPop, P2->totInd, P2->numberOfPlanets );
                ...
                for ( t = P2->shiptypes; t; t = t->next ) {
                    if ( ( t->flag ) || ( P eq P2 ) ) {
                        ...
                        fprintf( fpR,
                                 ...
                                 "        <cargo>%.2f</cargo>\n"
                                 "      </type>\n", safename( t->name ),
                                 t->drive, t->attacks, t->weapons, t->shields,
                                 t->cargo );
                    }
                }
            }
...
rRace( game *aGame, player *P, int level, int RorF )
{
    ...
    if ( nbr_msgs[0] || nbr_msgs[1] ) {
        ...
        if ( nbr_msgs[0] )
            rMessages( aGame, P, aGame->messages, "global", level + 4,
                       nbr_msgs[0] );
        if ( nbr_msgs[1] )
            rMessages( aGame, P, P->messages, "personal", level + 4,
                       nbr_msgs[1] );
        fprintf( fpR, "%s  </messages>\n", indent );
    }
            ...
            if ( r->who eq P ) {
                ...
                for ( plr = b->participants; plr; plr = plr->next ) {
                    ...
                    for ( g = plr->groups; g; g = g->next ) {
                        ...
                        fprintf( fpR,
                                 "%s          <velocity>%.2f</velocity>\n",
                                 indent, groupSpeed( g ) );
                        fprintf( fpR,
                                 "%s          <numAttacks>%.2f</numAttacks>\n",
                                 indent, g->attack );
                ...
                if ( P->flags & F_BATTLEPROTOCOL ) {
                    ...
                    for ( i = 0; i < b->protocol->cur; i++ ) {
                        ...
                        if ( s[i].result ) {
                            fprintf( fpR,
                                     "%s      <result>destroyed</result>\n",
                                     indent );
                        } else {
                            fprintf( fpR,
                                     "%s      <result>shields</result>\n",
                                     indent );
                        }
                        fprintf( fpR, "%s    </shot>\n", indent );
                    }
            ...
            for ( g = P2->groups; g; g = g->next ) {
                if ( groupLocation( aGame, g ) == NULL ) {
                    ...
                    fprintf( fpR, "%s      <x>%.2f</x>\n", indent,
                             ( groupx( aGame, g ) - P->mx ) / width );
                    fprintf( fpR, "%s      <y>%.2f</y>\n", indent,
                             ( groupy( aGame, g ) - P->my ) / height );
            ...
            if ( pt->owner == p2 && canseeplanet( P, pt ) ) {
                ...
                if ( pt->producing == PR_SHIP ) {
                    fprintf( fpR,
                             "%s    <production>%s</production>\n",
                             indent,
                             safename( pt->producingshiptype->name ) );
                } else {
                    fprintf( fpR,
                             "%s    <production>%s</production>\n",
    ...
    for ( pt = aGame->planets; pt; pt = pt->next ) {
        if ( isunidentified( P, pt ) ) {
            ...
            fprintf( fpR,
                     "%s    <position>\n%s      <x>%.2f</x>\n"
                     "%s      <y>%.2f</y>\n%s    </position>\n", indent,
                     indent, pt->x, indent, pt->y, indent );
            fprintf( fpR, "%s  </world>\n", indent );
        }
    }
    fprintf( fpR, "%s</unidentifiedWorlds>\n", indent );
...
rRoutes( game *aGame, player *P, int level )
{
    ...
    for ( pt = aGame->planets; pt; pt = pt->next ) {
        if ( pt->owner == P &&
             ( pt->routes[0] ||
               pt->routes[1] || pt->routes[2] || pt->routes[3] ) ) {
            int i;
            ...
            for ( i = 0; i < MAXCARGO; i++ ) {
                if ( pt->routes[i] )
                    fprintf( fpR, "%s    <%s>%s</%s>\n", indent,
                             loadtypename[i], safename( pt->routes[i]->name ),
                             loadtypename[i] );
            }
            fprintf( fpR, "%s  </route>\n", indent );
        }
                            ...
                            if ( P == P2 ) {
                                ...
                                if ( g->thefleet ) {
                                    ...
                                    fprintf( fpR, "%s    <fleet>%s</fleet>\n",
                                             indent,
                                             safename( g->thefleet->name ) );
                                } else {
                                    fprintf( fpR,
                                             "%s    <speed>%.2f</speed>\n",
                                             indent,
                                             g->type->drive * g->drive *
