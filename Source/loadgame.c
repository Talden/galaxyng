...
loadgame( char *gameName, int whichTurn )
{
    ...
    if ( whichTurn eq LG_CURRENT_TURN ) {
        sprintf( lineBuffer, "%s/data/%s/next_turn", galaxynghome,
                 aGame->name );
    ...
    for ( getLine( turnFile );
        ...
        if ( aGame->gameOptions.initial_drive < 0 ) {
            aGame->gameOptions.galactic_peace =
                aGame->gameOptions.galactic_peace * -1;
            aGame->gameOptions.initial_drive = getReadFloat(  );
        }
    ...
    for ( P = aGame->players; P; P = P->next ) {
        ...
        for ( getLine( turnFile );
            ...
            anAlliance->who =
                ( player * ) readPointer( ( list * ) aGame->players );
    ...
    for ( getLine( turnFile );
        ...
        aPlanet->owner =
            ( player * ) getReadPointer( ( list * ) aGame->players );
        ...
        if ( temp ) {
            ...
            aPlanet->producingshiptype =
                ( shiptype * ) readPointer( ( list * ) aPlanet->owner->
                                            shiptypes );
        } else
    ...
    for ( P = aGame->players; P; P = P->next ) {
        ...
        for ( getLine( turnFile );
            ...
            aGroup->next = NULL;
            aGroup->type =
                ( shiptype * ) readPointer( ( list * ) P->shiptypes );
